...
if ( ! class_exists( 'Gamers_Hub_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Gamers_Hub_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local gamers_hub_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $gamers_hub_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $gamers_hub_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$gamers_hub_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$gamers_hub_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$gamers_hub_font_family = trim( str_replace( array( "'", ';' ), '', $gamers_hub_font_family ) );
					$gamers_hub_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $gamers_hub_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $gamers_hub_font_family ] ) ) {
					$result[ $gamers_hub_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$gamers_hub_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$gamers_hub_font_family_url = $this->get_absolute_path( $gamers_hub_font_family_url );

					$result[ $gamers_hub_font_family ][] = $gamers_hub_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $gamers_hub_font_family ] = array_flip( array_flip( $result[ $gamers_hub_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Gamers_Hub_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'wptt_get_web_gamers_hub_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_web_gamers_hub_font_url( $url, $format = 'woff2' ) {
		$font = new Gamers_Hub_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
